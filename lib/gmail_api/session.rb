...
require 'gmail_api/api_client_fetcher'
...
module GmailApi
  ...
  class Session
    ...
    # Constructs a Gmail::Session object from OAuth2 credentials such as
    ...
    # Constructs a Gmail::Session object from OAuth2 access token string.
    ...
    # Returns an instance of Google::Apis::GmailV1::Service.
    def gmail_session
      @fetcher.gmail_session
    end
...
#needs adapting if necessary at all.
    def url_to_id(url)
      uri = URI.parse(url)
      if ['mail.google.com'].include?(uri.host)
        case uri.path
        #   # Document feed.
      ...
      fail(GmailApi::Error, 'The given URL is not a known Gmail URL: %s' % url)
    end
  end
end
